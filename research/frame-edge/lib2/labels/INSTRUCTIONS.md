# For this dataset (lib2: `library 2/300dpi`)

Everything below is the study's protocol, unchanged. Only the paths differ:

* crops are in `research/frame-edge/lib2/crops/label/`
* your labels go to `research/frame-edge/lib2/labels/<you>/<id>.json`
* before any `python` that calls `common.load`, set both, or you read the
  other dataset:

      export FRAME_EDGE_WORK=/Users/stefan.kurzella/github/reflecta-rps-7200-1/research/frame-edge/lib2
      export FRAME_EDGE_LIBRARY="/Users/stefan.kurzella/github/reflecta-rps-7200-1/library 2/300dpi"
      cd /Users/stefan.kurzella/github/reflecta-rps-7200-1/research/frame-edge
      uv run --no-sync python -c "from common import load; ..."

* The empty gate (`no_film`) appears here: on the frames at the strip's end
  the outer few columns read ~175 in every channel, neutral, far brighter than
  base. That sliver dominates the stretch, so the whole frame looks pale in
  view B and brown in view A -- the picture beside it is still readable.
