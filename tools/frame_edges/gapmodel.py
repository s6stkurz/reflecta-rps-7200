...
The roll (``ctx['roll']``) supplies two things, each from the *other*
...
from .units import pitch_columns
...
def roll_of(ctx: dict[str, Any]) -> Roll:
    ...
    # one `roll.Summary` per other frame, made once by `roll.summarise`
    for other in ctx.get("roll") or ():
        widths.extend(other.gap_widths)
        colours.extend(other.gap_colours)
    colour = None
    if colours:
